...
const ShoppingListPage = props => {
    ...
    const { shoppingList, isLoading, error } = useSelector(state => state.shoppingList);
    ...
    const totalPrice = shoppingList.reduce((acc, item) => acc + (item.amount * item.price), 0);
                        ...
                        : (
                            <div className={classes.root}>
                                        ...
                                        {shoppingList.map((product, index) =>
                                            <ListItem className={classes.listItem} key={product.uuid}>
                                                ...
                                                />
                                                {
                                                    ...
                                                    <select
                                                        name="size" id="size" className={classes.select}
                                                    ...
                                                    >
                                                        ...
                                                        </option>
                                                        {
                                                            product.sizes.map(size =>
                                                                isNaN(size) ?
                                                                    <option className={classes.options}>{capitalized(size)}</option>
                                                ...
                                                <Tooltip title="Delete">
                                                    <IconButton edge="end" aria-label="remove">
                                                        <DeleteIcon key={"delete-icon-" + product.uuid}
                                                ...
                                                <Tooltip title="Reduce">
                                                    <IconButton edge="end" aria-label="reduce">
                                                        <RemoveIcon key={"reduce-icon-" + index}
                                ...
                                >
                                    ...
                                    <DialogContent>
                                        <DialogContentText>
                                            {`Are you sure that you want to delete ${selectedProduct ? selectedProduct.caption : ''}?`}
                                        </DialogContentText>
                                    </DialogContent>
                                    <DialogActions>
                                        <Button onClick={() => onConfirm(selectedProduct ? selectedProduct.uuid : null)} color="secondary">Confirm</Button>
